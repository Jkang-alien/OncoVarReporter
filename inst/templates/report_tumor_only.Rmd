...
* Exclusion of calls that coincide with germline variants found in [Exome Aggregation Consortium]((http://exac.broadinstitute.org/)) (minor allele frequency >= `r report_data$maf_exclusion_threshold_exac` in any population)
* Exclusion of calls that coincide with germline variants found in [1000 Genomes Project](ftp://ftp.1000genomes.ebi.ac.uk/vol1/ftp/release/20130502/) (minor allele frequency >= `r report_data$maf_exclusion_threshold_1kg` in any population)
...
Total number of (unfiltered) calls: __`r report_data$unfiltered_n`__

Number of calls remaining after successive filtering steps:

1. Exclusion of variants found in ExAC (MAF in any population > `r report_data$maf_exclusion_threshold_exac`): __`r report_data$filter1_n`__ (__`r report_data$filter1_frac`__ % of all calls)
2. Exclusion of variants found in 1000 Genomes Project (MAF in any population > `r report_data$maf_exclusion_threshold_1kg`): __`r report_data$filter2_n`__ (__`r report_data$filter2_frac`__ % of all calls)
3. Exclusion of dbSNP (non-clinically-related) variants: __`r report_data$filter3_n`__ (__`r report_data$filter3_frac`__ % of all calls)
...
- A total of __`r nrow(report_data$variants_tier2_display)`__ unique variant(s) in the tumor sample are curated as disease-causing, predicted as driver mutations, or occur in previously detected mutation hotspots
<br>
...
- A total of __`r nrow(report_data$variants_tier3_display)`__ unique variant(s) in the tumor sample are found within known proto-oncogenes, tumor suppressor genes, or cancer census genes:
<br>
